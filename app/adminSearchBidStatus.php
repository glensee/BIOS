...
<body>
                        ...
                        <div class="ibox float-e-margins">
                            ...
                            <div class="ibox-content">
                                <div id="searchSuccess" class="alert alert-success" hidden>
                                    <strong>Success! </strong>Bid status records retrieved!
                                </div>
                                <div id="searchFailure" class="alert alert-danger" hidden>
                                    <!-- Error message is populated via jQuery below -->
                                </div>
                                <table class="table table-striped" id="resultsTableStatus" width="100%">
                                ...
                                <table class="table table-striped" id="resultsTableStudents" width="100%">
                                    <!-- Table is populated via AJAX below -->
                                    <thead>
                                        <tr>
                                            <th>User ID</th>
    ...
    function reset() {
        ...
        $("#searchFailure").html('');
        // Reset tables
        $("#resultsTableStatus").dataTable().fnClearTable();
        $("#resultsTableStatus").dataTable().fnDraw();
        $("#resultsTableStatus").dataTable().fnDestroy();
        $("#resultsTableStudents").dataTable().fnClearTable();
        $("#resultsTableStudents").dataTable().fnDraw();
        $("#resultsTableStudents").dataTable().fnDestroy();
    }
    ...
    function validateStatus(response) {
        if (response["status"] === "success") {
            ...
            $("#resultsTable").show();
            // Create DataTable
            createDataTableStatus(response);
            createDataTableStudents(response);
        } else {
            // Show Error Alert
            $("#searchSuccess").hide();
            $("#searchFailure").show();
    ...
    function createDataTableStatus(response) {
        // Populate jQuery DataTable
        table = $('#resultsTableStatus').DataTable({
    ...
    function createDataTableStudents(response) {
        // Populate jQuery DataTable
        table = $('#resultsTableStudents').DataTable({
    ...
    $("#searchForm").submit(function(e) {
        ...
        // Call POST request
        request = $.ajax({
            url: `json/bid-status.php?r=${jsonData}&token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6ImFkbWluIiwiZGF0ZXRpbWUiOiIyMDE5LTEwLTI5IDA4OjQ1OjQ2In0.3Udk97YTtixTEXkmpOMXfqlhGy1hmH7osmkxvIZHdcU&`,
            type: "post",
        });
